...
for i=1:length_input
 ...
 [gOpt,r,VOpt] = OptimalPolicySameEnvironmentsQuad(100,100,lambda,mu,theta,input(i)*q,[1 1]);
 
 m = mWhittleSameNoAbandonQuad(100,lambda,mu,thetaBis,input(i)*q,[1 1]);
 MatrixWhittle = IndexSameNoAbandon(m,mu);
 [gWhit] = PerfAnyPolSameQuad(100,100,lambda,mu,theta,input(i)*q,[1 1],MatrixWhittle);
 
 mMW = mMaxWeight(100,mu,[1 1]);
 MatrixMW = IndexMatrixSame(mMW);
 [gMW] = PerfAnyPolSameQuad(100,100,lambda,mu,theta,input(i)*q,[1 1],MatrixMW);
...
title('MaxWeight policy, scaling speed')
...
print(h,'MaxWeightSpeedSame','-dpdf','-r0')
